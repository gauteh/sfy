...
def test_filter_coeffs():
    ...
    CUTOFF = [1. / 30., 26.]  # Cut-off frequency for output
    FREQ = 208.  # Input frequency
    fir = sc.signal.firwin(NTAP, cutoff=CUTOFF, pass_zero='bandpass', fs=FREQ)
    ...
    NTAP = 129  # Length of filter
    CUTOFF = [1. / 30., 13.]  # Cut-off frequency for output
    FREQ = 208.  # Input frequency
    fir = sc.signal.firwin(NTAP, cutoff=CUTOFF, pass_zero='bandpass', fs=FREQ)
...
def test_fir_within_sin():
    NTAP = 129  # Length of filter
    CUTOFF = [1./30, 20.]  # Cut-off frequency for output
    FREQ = 208.  # Input frequency
    fir = sc.signal.firwin(NTAP, cutoff=CUTOFF, pass_zero='bandpass', fs=FREQ)
...
def test_fir_below_hp():
    NTAP = 1025  # Length of filter
    # CUTOFF = [1/10, 20.]  # Cut-off frequency for output
    CUTOFF = [0.1]  # Cut-off frequency for output
    FREQ = 208.  # Input frequency
    fir = sc.signal.firwin(NTAP, cutoff=CUTOFF, pass_zero='highpass', fs=FREQ)
    a, b = sc.signal.iirfilter(127, Wn=[0.1, 20], btype='bandpass', fs=FREQ)
    print(a, b)

    dt = 1. / FREQ
    t = np.arange(0, 4*4096) * dt

    s = 2 * np.sin(2 * t * 2 * np.pi)
    s3 = 2 * np.sin((1/100) * t * 2 * np.pi)
    # s3 = 0

    sf = s + s3
    # sf = sc.signal.detrend(sf)
    sf = np.convolve(sf, a, mode='valid')
    # sf = sf[NTAP//2:]

    import matplotlib.pyplot as plt
    plt.figure()
    plt.plot(np.arange(0,len(s))*dt, s, '-x')
    plt.plot((np.arange(0,len(sf))+64)*dt, sf, '-x')
    plt.show()
