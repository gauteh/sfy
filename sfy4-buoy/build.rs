...
fn main() {
    ...
    // `EGPS_POSITION_INTERVAL`, and separately runs a high-rate batch
    // (IMU/GPS, `EGPS_BATCH_DURATION` seconds long, 20 min by default) every
    ...
    // Length of a high-rate egps batch burst. Defaults to 20 min, matching
    // the axl/IMU `spectrum` feature's fixed 20-minute window
    // (`waves::welch::Welch`) -- there's normally no reason to run this any
    // other length, but it's build-time configurable (e.g. for a short
    // debug binary that duty-cycles much faster than a real deployment)
    // rather than requiring a source edit each time.
    ...
    if egps_batch_period < egps_batch_duration + egps_position_interval {
        println!(
